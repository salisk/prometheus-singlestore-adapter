...
type FunctionCall func(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector

// === time() float64 ===
func funcTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return promql.Vector{promql.Sample{Point: promql.Point{
...
func extrapolatedRate(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper, isCounter, isRate bool) promql.Vector {
	ms := args[0].(*parser.MatrixSelector)
	vs := ms.VectorSelector.(*parser.VectorSelector)
	var (
		samples    = vals[0].(promql.Matrix)[0]
	...
	return append(enh.Out, promql.Sample{
		Point: promql.Point{V: resultValue},
	})
}

// === delta(Matrix parser.ValueTypeMatrix) Vector ===
func funcDelta(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return extrapolatedRate(vals, args, enh, false, false)
}

// === rate(node parser.ValueTypeMatrix) Vector ===
func funcRate(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return extrapolatedRate(vals, args, enh, true, true)
}

// === increase(node parser.ValueTypeMatrix) Vector ===
func funcIncrease(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return extrapolatedRate(vals, args, enh, true, false)
}

// === irate(node parser.ValueTypeMatrix) Vector ===
func funcIrate(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return instantValue(vals, enh.Out, true)
}

// === idelta(node model.ValMatrix) Vector ===
func funcIdelta(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return instantValue(vals, enh.Out, false)
}

func instantValue(vals []parser.Value, out promql.Vector, isRate bool) promql.Vector {
	samples := vals[0].(promql.Matrix)[0]
	// No sense in trying to compute a rate without at least two points. Drop
	// this Vector element.
	if len(samples.Points) < 2 {
	...
	return append(out, promql.Sample{
		Point: promql.Point{V: resultValue},
	})
}
...
func funcHoltWinters(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	samples := vals[0].(promql.Matrix)[0]

	// The smoothing factor argument.
	sf := vals[1].(promql.Vector)[0].V

	// The trend factor argument.
	tf := vals[2].(promql.Vector)[0].V
	...
	return append(enh.Out, promql.Sample{
		Point: promql.Point{V: s1},
	})
}

// === sort(node parser.ValueTypeVector) Vector ===
func funcSort(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	// NaN should sort to the bottom, so take descending sort with NaN first and
	// reverse it.
	byValueSorter := vectorByReverseValueHeap(vals[0].(promql.Vector))
	sort.Sort(sort.Reverse(byValueSorter))
	return promql.Vector(byValueSorter)
}

// === sortDesc(node parser.ValueTypeVector) Vector ===
func funcSortDesc(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	// NaN should sort to the bottom, so take ascending sort with NaN first and
	// reverse it.
	byValueSorter := vectorByValueHeap(vals[0].(promql.Vector))
	sort.Sort(sort.Reverse(byValueSorter))
	return promql.Vector(byValueSorter)
}

// === clamp(Vector parser.ValueTypeVector, min, max Scalar) Vector ===
func funcClamp(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	vec := vals[0].(promql.Vector)
	min := vals[1].(promql.Vector)[0].Point.V
	max := vals[2].(promql.Vector)[0].Point.V
	if max < min {
		return enh.Out
	}
	for _, el := range vec {
		enh.Out = append(enh.Out, promql.Sample{
			...
			Point:  promql.Point{V: math.Max(min, math.Min(max, el.V))},
		})
	}
	return enh.Out
}
...
func funcClampMax(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	vec := vals[0].(promql.Vector)
	max := vals[1].(promql.Vector)[0].Point.V
	for _, el := range vec {
		enh.Out = append(enh.Out, promql.Sample{
			Metric: enh.DropMetricName(el.Metric),
			Point:  promql.Point{V: math.Min(max, el.V)},
		})
	}
	return enh.Out
}
...
func funcClampMin(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	vec := vals[0].(promql.Vector)
	min := vals[1].(promql.Vector)[0].Point.V
	for _, el := range vec {
		enh.Out = append(enh.Out, promql.Sample{
			Metric: enh.DropMetricName(el.Metric),
			Point:  promql.Point{V: math.Max(min, el.V)},
		})
	}
	return enh.Out
}
...
func funcRound(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	vec := vals[0].(promql.Vector)
	// round returns a number rounded to toNearest.
	// Ties are solved by rounding up.
	toNearest := float64(1)
	if len(args) >= 2 {
		toNearest = vals[1].(promql.Vector)[0].Point.V
	}
...
func funcScalar(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	v := vals[0].(promql.Vector)
	if len(v) != 1 {
		return append(enh.Out, promql.Sample{
			Point: promql.Point{V: math.NaN()},
		})
	}
	return append(enh.Out, promql.Sample{
		Point: promql.Point{V: v[0].V},
	})
}

func aggrOverTime(vals []parser.Value, enh *EvalNodeHelper, aggrFn func([]promql.Point) float64) promql.Vector {
	el := vals[0].(promql.Matrix)[0]

	return append(enh.Out, promql.Sample{
		Point: promql.Point{V: aggrFn(el.Points)},
	})
}

// === avg_over_time(Matrix parser.ValueTypeMatrix) Vector ===
func funcAvgOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
...
func funcCountOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return aggrOverTime(vals, enh, func(values []promql.Point) float64 {
...
func funcLastOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	el := vals[0].(promql.Matrix)[0]

	return append(enh.Out, promql.Sample{
		Metric: el.Metric,
		Point:  promql.Point{V: el.Points[len(el.Points)-1].V},
	})
}

// === max_over_time(Matrix parser.ValueTypeMatrix) Vector ===
func funcMaxOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return aggrOverTime(vals, enh, func(values []promql.Point) float64 {
		max := values[0].V
		for _, v := range values {
...
func funcMinOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return aggrOverTime(vals, enh, func(values []promql.Point) float64 {
		min := values[0].V
		for _, v := range values {
...
func funcSumOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return aggrOverTime(vals, enh, func(values []promql.Point) float64 {
		var sum, c float64
		for _, v := range values {
...
func funcQuantileOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	q := vals[0].(promql.Vector)[0].V
	el := vals[1].(promql.Matrix)[0]
	...
	return append(enh.Out, promql.Sample{
		Point: promql.Point{V: quantile(q, values)},
	})
}

// === stddev_over_time(Matrix parser.ValueTypeMatrix) Vector ===
func funcStddevOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return aggrOverTime(vals, enh, func(values []promql.Point) float64 {
		var count float64
...
func funcStdvarOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return aggrOverTime(vals, enh, func(values []promql.Point) float64 {
		var count float64
		var mean, cMean float64
		var aux, cAux float64
...
func funcAbsent(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	if len(vals[0].(promql.Vector)) > 0 {
		return enh.Out
	}
...
func funcAbsentOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return append(enh.Out,
		promql.Sample{
			Point: promql.Point{V: 1},
		})
...
func funcPresentOverTime(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return aggrOverTime(vals, enh, func(values []promql.Point) float64 {
		return 1
	})
}

func simpleFunc(vals []parser.Value, enh *EvalNodeHelper, f func(float64) float64) promql.Vector {
	for _, el := range vals[0].(promql.Vector) {
		enh.Out = append(enh.Out, promql.Sample{
			Metric: enh.DropMetricName(el.Metric),
			Point:  promql.Point{V: f(el.V)},
		})
	}
	return enh.Out
}
...
func funcAbs(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Abs)
}

// === ceil(Vector parser.ValueTypeVector) Vector ===
func funcCeil(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Ceil)
}

// === floor(Vector parser.ValueTypeVector) Vector ===
func funcFloor(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Floor)
}

// === exp(Vector parser.ValueTypeVector) Vector ===
func funcExp(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Exp)
}

// === sqrt(Vector VectorNode) Vector ===
func funcSqrt(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Sqrt)
}

// === ln(Vector parser.ValueTypeVector) Vector ===
func funcLn(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Log)
}

// === log2(Vector parser.ValueTypeVector) Vector ===
func funcLog2(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Log2)
}

// === log10(Vector parser.ValueTypeVector) Vector ===
func funcLog10(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Log10)
}

// === sin(Vector parser.ValueTypeVector) Vector ===
func funcSin(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Sin)
}

// === cos(Vector parser.ValueTypeVector) Vector ===
func funcCos(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Cos)
}

// === tan(Vector parser.ValueTypeVector) Vector ===
func funcTan(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Tan)
}

// == asin(Vector parser.ValueTypeVector) Vector ===
func funcAsin(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Asin)
}

// == acos(Vector parser.ValueTypeVector) Vector ===
func funcAcos(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Acos)
}

// == atan(Vector parser.ValueTypeVector) Vector ===
func funcAtan(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Atan)
}

// == sinh(Vector parser.ValueTypeVector) Vector ===
func funcSinh(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Sinh)
}

// == cosh(Vector parser.ValueTypeVector) Vector ===
func funcCosh(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Cosh)
}

// == tanh(Vector parser.ValueTypeVector) Vector ===
func funcTanh(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Tanh)
}

// == asinh(Vector parser.ValueTypeVector) Vector ===
func funcAsinh(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Asinh)
}

// == acosh(Vector parser.ValueTypeVector) Vector ===
func funcAcosh(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Acosh)
}

// == atanh(Vector parser.ValueTypeVector) Vector ===
func funcAtanh(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, math.Atanh)
}

// === rad(Vector parser.ValueTypeVector) Vector ===
func funcRad(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
...
func funcDeg(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, func(v float64) float64 {
...
// === pi() Scalar ===
func funcPi(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return promql.Vector{promql.Sample{Point: promql.Point{
		V: math.Pi,
	}}}
}
...
func funcSgn(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return simpleFunc(vals, enh, func(v float64) float64 {
		if v < 0 {
			return -1
		} else if v > 0 {
...
func funcTimestamp(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	vec := vals[0].(promql.Vector)
	for _, el := range vec {
		enh.Out = append(enh.Out, promql.Sample{
			Metric: enh.DropMetricName(el.Metric),
			Point:  promql.Point{V: float64(el.T) / 1000},
		})
	}
	return enh.Out
}
...
func funcDeriv(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	samples := vals[0].(promql.Matrix)[0]
	...
	return append(enh.Out, promql.Sample{
		Point: promql.Point{V: slope},
	})
}

// === predict_linear(node parser.ValueTypeMatrix, k parser.ValueTypeScalar) Vector ===
func funcPredictLinear(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	samples := vals[0].(promql.Matrix)[0]
	duration := vals[1].(promql.Vector)[0].V
	...
	return append(enh.Out, promql.Sample{
		Point: promql.Point{V: slope*duration + intercept},
	})
}

// === histogram_quantile(k parser.ValueTypeScalar, Vector parser.ValueTypeVector) Vector ===
func funcHistogramQuantile(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	q := vals[0].(promql.Vector)[0].V
	inVec := vals[1].(promql.Vector)
	...
	for _, mb := range enh.signatureToMetricWithBuckets {
		if len(mb.buckets) > 0 {
			enh.Out = append(enh.Out, promql.Sample{
				Metric: mb.metric,
				Point:  promql.Point{V: bucketQuantile(q, mb.buckets)},
			})
		}
	}
...
func funcResets(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	samples := vals[0].(promql.Matrix)[0]
	...
	return append(enh.Out, promql.Sample{
		Point: promql.Point{V: float64(resets)},
	})
}

// === changes(Matrix parser.ValueTypeMatrix) Vector ===
func funcChanges(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	samples := vals[0].(promql.Matrix)[0]
	...
	return append(enh.Out, promql.Sample{
		Point: promql.Point{V: float64(changes)},
	})
}

// === label_replace(Vector parser.ValueTypeVector, dst_label, replacement, src_labelname, regex parser.ValueTypeString) Vector ===
func funcLabelReplace(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	var (
		vector   = vals[0].(promql.Vector)
	...
	for _, el := range vector {
		...
		enh.Out = append(enh.Out, promql.Sample{
			Metric: outMetric,
			Point:  promql.Point{V: el.Point.V},
		})
	}
	return enh.Out
}
...
func funcVector(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return append(enh.Out,
		promql.Sample{
			Metric: labels.Labels{},
			Point:  promql.Point{V: vals[0].(promql.Vector)[0].V},
		})
}

// === label_join(vector model.ValVector, dest_labelname, separator, src_labelname...) Vector ===
func funcLabelJoin(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	var (
		vector    = vals[0].(promql.Vector)
		dst       = stringFromArg(args[1])
...
func dateWrapper(vals []parser.Value, enh *EvalNodeHelper, f func(time.Time) float64) promql.Vector {
	if len(vals) == 0 {
		return append(enh.Out,
			promql.Sample{
				Metric: labels.Labels{},
				Point:  promql.Point{V: f(time.Unix(enh.Ts/1000, 0).UTC())},
			})
	}
...
func funcDaysInMonth(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
...
func funcDayOfMonth(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return dateWrapper(vals, enh, func(t time.Time) float64 {
...
func funcDayOfWeek(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return dateWrapper(vals, enh, func(t time.Time) float64 {
...
func funcDayOfYear(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return dateWrapper(vals, enh, func(t time.Time) float64 {
...
func funcHour(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return dateWrapper(vals, enh, func(t time.Time) float64 {
...
func funcMinute(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return dateWrapper(vals, enh, func(t time.Time) float64 {
...
func funcMonth(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return dateWrapper(vals, enh, func(t time.Time) float64 {
...
func funcYear(vals []parser.Value, args parser.Expressions, enh *EvalNodeHelper) promql.Vector {
	return dateWrapper(vals, enh, func(t time.Time) float64 {
