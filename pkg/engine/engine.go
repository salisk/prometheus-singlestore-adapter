...
type Query interface {
	...
	Exec(ctx context.Context) *promql.Result
...
type query struct {
	...
	queryable s2prometheus.Querier
...
func (q *query) Exec(ctx context.Context) *promql.Result {
	...
	return &promql.Result{Err: err, Value: res, Warnings: warnings}
}
...
func (ng *Engine) NewInstantQuery(q s2prometheus.Querier, opts *QueryOpts, qs string, ts time.Time) (Query, error) {
...
func (ng *Engine) NewRangeQuery(q s2prometheus.Querier, opts *QueryOpts, qs string, start, end time.Time, interval time.Duration) (Query, error) {
	expr, err := parser.ParseExpr(qs)
	if err != nil {
		return nil, err
	}
...
func (ng *Engine) newQuery(q s2prometheus.Querier, opts *QueryOpts, expr parser.Expr, start, end time.Time, interval time.Duration) (*query, error) {
...
func (ng *Engine) execEvalStmt(ctx context.Context, query *query, s *parser.EvalStmt) (parser.Value, storage.Warnings, error) {
	...
	topNodes := ng.populateSeries(query.queryable, s)
	...
	fmt.Printf("[Promql] start: %s, end: %s, interval: %s\n", s.Start, s.End, s.Interval)
	if s.Start == s.End && s.Interval == 0 {
		...
		case parser.ValueTypeVector:
			...
			vector := make(promql.Vector, len(mat))
			for i, s := range mat {
				// Point might have a different timestamp, force it to the evaluation
				// timestamp as that is when we ran the evaluation.
				vector[i] = promql.Sample{Metric: s.Metric, Point: promql.Point{V: s.Points[0].V, T: start}}
			}
			return vector, warnings, nil
		case parser.ValueTypeScalar:
			return promql.Scalar{V: mat[0].Points[0].V, T: start}, warnings, nil
...
func (ng *Engine) populateSeries(querier s2prometheus.Querier, s *parser.EvalStmt) map[parser.Node]struct{} {
	var (
		// Whenever a MatrixSelector is evaluated, evalRange is set to the corresponding range.
		// The evaluation of the VectorSelector inside then evaluates the given range and unsets
		// the variable.
	...
	parser.Inspect(s.Expr, func(node parser.Node, path []parser.Node) error {
		...
		case *parser.VectorSelector:
			...
			set := querier.Select(false, hints, n.LabelMatchers...)
...
type EvalNodeHelper struct {
	...
	rightSigs    map[string]promql.Sample
...
func (ev *evaluator) rangeEval(prepSeries func(labels.Labels, *EvalSeriesHelper), funcCall func([]parser.Value, [][]EvalSeriesHelper, *EvalNodeHelper) (promql.Vector, storage.Warnings), exprs ...parser.Expr) (promql.Matrix, storage.Warnings) {
	...
	enh := &EvalNodeHelper{Out: make(promql.Vector, 0, biggestLen)}
	...
	mat := make(promql.Matrix, 0, len(seriess))
...
func (ev *evaluator) evalSubquery(subq *parser.SubqueryExpr) (*parser.MatrixSelector, int, storage.Warnings) {
	...
	for _, s := range mat {
		...
		vs.Series = append(vs.Series, promql.NewStorageSeries(s))
	}
	return ms, totalSamples, ws
}

func (ev *evaluator) getPushdownResult(vs *parser.VectorSelector, numSteps int, dropName bool) promql.Matrix {
	mat := make(promql.Matrix, 0, len(vs.Series))
	for i, s := range vs.Series {
		...
		for it.Next() {
			t, v := it.At()
			ss.Points = append(ss.Points, promql.Point{V: v, T: t})
		}
		if len(ss.Points) > 0 {
			mat = append(mat, ss)
		} else {
...
func (ev *evaluator) eval(expr parser.Expr) (parser.Value, storage.Warnings) {
	...
	if _, isTopNode := ev.topNodes[expr]; isTopNode {
		...
		parser.Inspect(expr, func(node parser.Node, path []parser.Node) error {
			fmt.Printf("[Engine] top node, inspect node.type: %s\n", node.String())
			switch n := node.(type) {
	...
	fmt.Printf("[Engine] expression type: %s, expression: %s\n", expr.Type(), expr.String())

	switch e := expr.(type) {
	case *parser.AggregateExpr:
		fmt.Println("[Engine] aggregate expr")
		...
		if s, ok := param.(*parser.StringLiteral); ok {
			return ev.rangeEval(initSeries, func(v []parser.Value, sh [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
				fmt.Println("[Engine] string literal")
				return ev.aggregation(e.Op, sortedGrouping, e.Without, s.Val, v[0].(promql.Vector), sh[0], enh), nil
			}, e.Expr)
		}
		return ev.rangeEval(initSeries, func(v []parser.Value, sh [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
			var param float64
			if e.Param != nil {
				param = v[0].(promql.Vector)[0].V
			}
			return ev.aggregation(e.Op, sortedGrouping, e.Without, param, v[1].(promql.Vector), sh[1], enh), nil
		}, e.Param, e.Expr)

	case *parser.Call:
		fmt.Println("[Engine] function call")
		...
		if e.Func.Name == "timestamp" {
			...
			if ok {
				return ev.rangeEval(nil, func(v []parser.Value, _ [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
					if vs.Timestamp != nil {
		...
		otherArgs := make([]promql.Matrix, len(e.Args))
		otherInArgs := make([]promql.Vector, len(e.Args))
		for i, e := range e.Args {
			if i != matrixArgIndex {
				val, ws := ev.eval(e)
				otherArgs[i] = val.(promql.Matrix)
				otherInArgs[i] = promql.Vector{promql.Sample{}}
		...
		mat := make(promql.Matrix, 0, len(selVS.Series)) // Output matrix.
		...
		inMatrix := make(promql.Matrix, 1)
		inArgs[matrixArgIndex] = inMatrix
		enh := &EvalNodeHelper{Out: make(promql.Vector, 0, 1)}
		...
		for i, s := range selVS.Series {
			...
			for ts, step := ev.startTimestamp, -1; ts <= ev.endTimestamp; ts += ev.interval {
				...
				enh.Out = outVec[:0]
				if len(outVec) > 0 {
					ss.Points = append(ss.Points, promql.Point{V: outVec[0].Point.V, T: ts})
				}
		...
		if e.Func.Name == "absent_over_time" {
			...
			newp := make([]promql.Point, 0, steps-len(found))
			for ts := ev.startTimestamp; ts <= ev.endTimestamp; ts += ev.interval {
				if _, ok := found[ts]; !ok {
					newp = append(newp, promql.Point{T: ts, V: 1})
				}
			}
	...
	case *parser.BinaryExpr:
		...
		case lt == parser.ValueTypeScalar && rt == parser.ValueTypeScalar:
			return ev.rangeEval(nil, func(v []parser.Value, _ [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
				val := scalarBinop(e.Op, v[0].(promql.Vector)[0].Point.V, v[1].(promql.Vector)[0].Point.V)
				return append(enh.Out, promql.Sample{Point: promql.Point{V: val}}), nil
			}, e.LHS, e.RHS)
		case lt == parser.ValueTypeVector && rt == parser.ValueTypeVector:
			...
			switch e.Op {
			case parser.LAND:
				return ev.rangeEval(initSignatures, func(v []parser.Value, sh [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
					return ev.VectorAnd(v[0].(promql.Vector), v[1].(promql.Vector), e.VectorMatching, sh[0], sh[1], enh), nil
				}, e.LHS, e.RHS)
			case parser.LOR:
				return ev.rangeEval(initSignatures, func(v []parser.Value, sh [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
					return ev.VectorOr(v[0].(promql.Vector), v[1].(promql.Vector), e.VectorMatching, sh[0], sh[1], enh), nil
				}, e.LHS, e.RHS)
			case parser.LUNLESS:
				return ev.rangeEval(initSignatures, func(v []parser.Value, sh [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
					return ev.VectorUnless(v[0].(promql.Vector), v[1].(promql.Vector), e.VectorMatching, sh[0], sh[1], enh), nil
				}, e.LHS, e.RHS)
			default:
				return ev.rangeEval(initSignatures, func(v []parser.Value, sh [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
					return ev.VectorBinop(e.Op, v[0].(promql.Vector), v[1].(promql.Vector), e.VectorMatching, e.ReturnBool, sh[0], sh[1], enh), nil
				}, e.LHS, e.RHS)
			}

		case lt == parser.ValueTypeVector && rt == parser.ValueTypeScalar:
			return ev.rangeEval(nil, func(v []parser.Value, _ [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
				return ev.VectorscalarBinop(e.Op, v[0].(promql.Vector), promql.Scalar{V: v[1].(promql.Vector)[0].Point.V}, false, e.ReturnBool, enh), nil
			}, e.LHS, e.RHS)

		case lt == parser.ValueTypeScalar && rt == parser.ValueTypeVector:
			return ev.rangeEval(nil, func(v []parser.Value, _ [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
				return ev.VectorscalarBinop(e.Op, v[1].(promql.Vector), promql.Scalar{V: v[0].(promql.Vector)[0].Point.V}, true, e.ReturnBool, enh), nil
			}, e.LHS, e.RHS)
		}

	case *parser.NumberLiteral:
		return ev.rangeEval(nil, func(v []parser.Value, _ [][]EvalSeriesHelper, enh *EvalNodeHelper) (promql.Vector, storage.Warnings) {
			return append(enh.Out, promql.Sample{Point: promql.Point{V: e.Val}}), nil
		})

	case *parser.StringLiteral:
		fmt.Println("[Engine] string literal expr")
		return promql.String{V: e.Val, T: ev.startTimestamp}, nil

	case *parser.VectorSelector:
		fmt.Println("[Engine] vector selector")

		ws, err := checkAndExpandSeriesSet(ev.ctx, e)
		if err != nil {
			ev.error(errWithWarnings{fmt.Errorf("expanding series: %w", err), ws})
		}
		mat := make(promql.Matrix, 0, len(e.Series))
		it := storage.NewMemoizedEmptyIterator(durationMilliseconds(ev.lookbackDelta))
		for i, s := range e.Series {
			fmt.Printf("[Engine] vector selector series %d: %v\n", i, s)
			...
			fmt.Printf("[Engine] points %d: %v\n", i, ss.Points)
			fmt.Printf("[Engine] start: %d, end: %d, interval: %d\n", ev.startTimestamp, ev.endTimestamp, ev.interval)

			for ts, step := ev.startTimestamp, -1; ts <= ev.endTimestamp; ts += ev.interval {
				step++
				_, v, ok := ev.vectorSelectorSingle(it, e, ts)
				fmt.Printf("[Engine] step %v, ok: %d\n", v, ok)
				if ok {
					if ev.currentSamples < ev.maxSamples {
						ss.Points = append(ss.Points, promql.Point{V: v, T: ts})
			...
			fmt.Printf("[Engine] vector selector series points %d: %v\n", i, ss.Points)
		...
		ev.samplesStats.UpdatePeak(ev.currentSamples)
		fmt.Printf("[Engine] vector selector: %v, string: %s, err: %v\n", mat, mat.String(), ws)
		return mat, ws
	...
	case *parser.StepInvariantExpr:
		...
		for i := range mat {
			...
			for ts := ev.startTimestamp + ev.interval; ts <= ev.endTimestamp; ts = ts + ev.interval {
				mat[i].Points = append(mat[i].Points, promql.Point{
					T: ts,
					V: mat[i].Points[0].V,
				})
				ev.currentSamples++
...
func (ev *evaluator) vectorSelector(node *parser.VectorSelector, ts int64) (promql.Vector, storage.Warnings) {
	ws, err := checkAndExpandSeriesSet(ev.ctx, node)
	if err != nil {
		ev.error(errWithWarnings{fmt.Errorf("expanding series: %w", err), ws})
	}
	vec := make(promql.Vector, 0, len(node.Series))
	it := storage.NewMemoizedEmptyIterator(durationMilliseconds(ev.lookbackDelta))
...
func (ev *evaluator) vectorSelectorSingle(it *storage.MemoizedSeriesIterator, node *parser.VectorSelector, ts int64) (int64, float64, bool) {
	...
	fmt.Printf("[VectorSelectorSingle] refTime: %d, ok: %v, t: %d, v: %f\n", refTime, ok, t, v)
...
func getPointSlice(sz int) []promql.Point {
	p := pointPool.Get()
	if p != nil {
		return p.([]promql.Point)
	}
	return make([]promql.Point, 0, sz)
}

func putPointSlice(p []promql.Point) {
	//nolint:staticcheck // Ignore SA6002 relax staticcheck verification.
	pointPool.Put(p[:0])
}

// matrixSelector evaluates a *parser.MatrixSelector expression.
func (ev *evaluator) matrixSelector(node *parser.MatrixSelector) (promql.Matrix, storage.Warnings) {
	var (
		...
		matrix = make(promql.Matrix, 0, len(vs.Series))
...
func (ev *evaluator) matrixIterSlice(it *storage.BufferedSeriesIterator, mint, maxt int64, out []promql.Point) []promql.Point {
...
func (ev *evaluator) VectorAnd(lhs, rhs promql.Vector, matching *parser.VectorMatching, lhsh, rhsh []EvalSeriesHelper, enh *EvalNodeHelper) promql.Vector {
...
func (ev *evaluator) VectorOr(lhs, rhs promql.Vector, matching *parser.VectorMatching, lhsh, rhsh []EvalSeriesHelper, enh *EvalNodeHelper) promql.Vector {
	if matching.Card != parser.CardManyToMany {
		panic("set operations must only use many-to-many matching")
	}
...
func (ev *evaluator) VectorUnless(lhs, rhs promql.Vector, matching *parser.VectorMatching, lhsh, rhsh []EvalSeriesHelper, enh *EvalNodeHelper) promql.Vector {
	if matching.Card != parser.CardManyToMany {
		panic("set operations must only use many-to-many matching")
	}
...
func (ev *evaluator) VectorBinop(op parser.ItemType, lhs, rhs promql.Vector, matching *parser.VectorMatching, returnBool bool, lhsh, rhsh []EvalSeriesHelper, enh *EvalNodeHelper) promql.Vector {
	if matching.Card == parser.CardManyToMany {
	...
	if enh.rightSigs == nil {
		enh.rightSigs = make(map[string]promql.Sample, len(enh.Out))
	} else {
...
func (ev *evaluator) VectorscalarBinop(op parser.ItemType, lhs promql.Vector, rhs promql.Scalar, swap, returnBool bool, enh *EvalNodeHelper) promql.Vector {
...
// must be sorted.
func (ev *evaluator) aggregation(op parser.ItemType, grouping []string, without bool, param interface{}, vec promql.Vector, seriesHelper []EvalSeriesHelper, enh *EvalNodeHelper) promql.Vector {
		...
		case parser.BOTTOMK:
			if int64(len(group.reverseHeap)) < k || group.reverseHeap[0].V > s.V || math.IsNaN(group.reverseHeap[0].V) {
				if int64(len(group.reverseHeap)) == k {
					if k == 1 { // For k==1 we can replace in-situ.
						group.reverseHeap[0] = promql.Sample{
							Point:  promql.Point{V: s.V},
							Metric: s.Metric,
						}
						break
	...
	for _, aggr := range orderedResult {
		...
		enh.Out = append(enh.Out, promql.Sample{
			Metric: aggr.labels,
			Point:  promql.Point{V: aggr.value},
		})
	}
	return enh.Out
}
